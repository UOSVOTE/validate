...
;(async () => {
    ...
    // fs.writeFileSync("PK", savedPK);
    // const secretKey = keyGenerator.secretKey()
    // const savedSK = secretKey.save();
    // console.log(savedSK);
    // fs.writeFileSync("SK", savedSK);
    const savedSK = fs.readFileSync("SK");
    const savedPK = fs.readFileSync("PK");
// 
    const secretKey = seal.SecretKey();
    secretKey.load(context, savedSK);
    ...
    // const array = Int32Array.from([1,0,0,0,0])
    ...
    // const newCipher = encryptor.encrypt(encoder.encode(Int32Array.from([0])))
    // for(let i=0; i< 1000; ++i){
    //     // console.log(i);
    //     evaluator.add(newCipher, cipherText, newCipher) // Op (A), Op (B), Op (Dest)
    // }
    ...
    // const savedCipher = newCipher.save();
    // fs.writeFileSync("cipher", savedCipher);
    // Decrypt the CipherText
    const newCipher = seal.CipherText();
    const savedCipher= fs.readFileSync("cipher");
    newCipher.load(context, savedCipher);
    const decryptedPlainText = decryptor.decrypt(newCipher)
